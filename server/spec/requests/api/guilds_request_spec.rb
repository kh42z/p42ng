...
describe "Guild", type: :request do
  ...
  describe "#members" do
    ...
    it 'should let officers add members' do
      ...
      post members_api_guild_url(Guild.first, tid: user_2.id), headers: user_1_access
    ...
    it 'should return error => param missing' do
      ...
      expect(json["error"]).to eq "param is missing or the value is empty: tid"
    end
    it 'should not add a member who is already member of a guild' do
      ...
      post members_api_guild_url(Guild.first, tid: user_1.id), headers: access_token
      post members_api_guild_url(Guild.last, tid: user_1.id), headers: access_token_2
      expect(Guild.last.members.count).to eq 1
      expect(User.find(user_1.id).guild).to eq Guild.first
    end
    it 'should destroy a member' do
      post members_api_guild_url(Guild.first, tid: user_1.id), headers: access_token
      delete members_api_guild_url(Guild.first, tid: user_1.id), headers: access_token
    ...
    it 'should not destroy a member of another guild' do
      post api_guilds_url, headers: access_token_2, params: attributes_2
      post members_api_guild_url(Guild.first, tid: user_1.id), headers: access_token
      delete members_api_guild_url(Guild.last, tid: user_1.id), headers: access_token_2
      expect(User.find( user_1.id).guild).to eq Guild.first
      expect(response.status).to eq 422
    end
    context 'if owner leaves' do
      it 'should destroy guild if he is the last to leave' do
        delete members_api_guild_url(Guild.first, tid: auth.id), headers: access_token
        expect(response.status).to eq 204
  ...
  describe "#officers" do
    ...
    it 'should not add an officer who is already officer in another guild' do
      post officers_api_guild_url(Guild.first, tid: user_1.id), headers: access_token
      post api_guilds_url, headers: access_token_2, params: attributes_2
      post officers_api_guild_url(Guild.last, tid: user_1.id), headers: access_token_2
      expect(Guild.last.officers.count).to eq 0
      expect(response.message).to eq 'Not Found'
    end
    it '(owner) should destroy an officer' do
      post officers_api_guild_url(Guild.first, tid: user_1.id), headers: access_token
      delete officers_api_guild_url(Guild.first, tid: user_1.id), headers: access_token
    ...
    it 'should not destroy an officer of another guild' do
      post officers_api_guild_url(Guild.first, tid: user_1.id), headers: access_token
      post api_guilds_url, headers: access_token_2, params: attributes_2
      delete officers_api_guild_url(Guild.last, tid: user_1.id), headers: access_token_2
      expect(response.message).to eq 'Not Found'
      expect(Guild.first.officers.count).to eq 1
    end
  end
