  ...
  describe '#game_point_assignment' do
    context 'if user has a guild' do
      ...
      it "should assign points to his guild score (alan)" do
        game_2 = create(:game, player_left: tom, player_right: alan, winner: alan)
        expect{ game_point_assignment(game_2) }.to change{ Guild.last.score }.by(10)
      end
      context 'in war and duels a war enemy' do
        before {
          attributes = { from: nos.id, on: bang.id, war_start: DateTime.now, war_end: DateTime.new(2022, 01, 01, 00, 00, 0), prize: 1000, max_unanswered: 10, from_score: 0, on_score: 0 }
